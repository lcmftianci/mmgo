package com.example.myapplication.mario.graphics;
...
public class MarioAnimation extends CharacterAnimation {
	...
	public Animation getAnimation(MovingActor.State state, int level) {
		switch(state) {
			case Walking:
				if(level == 1) {
					return walking;
	...
	public Vector2 getDimensions(MovingActor.State state, int level) {
		switch(state) {
			case Walking:
				if(level == 1) {
