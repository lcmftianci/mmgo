...
import com.example.myapplication.mario.model.Flag;
import com.example.myapplication.mario.model.Mario;
import com.example.myapplication.mario.model.MovingActor;
...
public class MarioActions extends Actions {
	...
	public static Action setStateAction(Mario actor, MovingActor.State state) {
		return new setState(actor, state);
	}

	static public class setState extends Action {
		private MovingActor.State state;
		
		public setState(Mario actor, MovingActor.State state) {
			this.actor = actor;
			this.state = state;
		}
	...
	static public class walkTo extends Action {
		...
		public boolean act(float delta) {
			if(actor.getX() < x) {
				((Mario) actor).move(MovingActor.Direction.RIGHT);
				return false;
			}
			return true;
		}
