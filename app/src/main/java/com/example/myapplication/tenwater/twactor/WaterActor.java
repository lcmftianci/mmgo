...
public class WaterActor extends Actor {
    ...
    private void createSmallWaterAfterExplode() {
        if (getX() - getWidth() / 2.0f > group.getGridImage().getX()) {
            SmallWaterActor smallWaterActor = new SmallWaterActor(group.getGridImage(), SmallWaterActor.SmallWaterMoveDirection.LEFT);
            setSmallWaterActorProperty(smallWaterActor);
            System.out.println("L");
        }
        
        if (getX() + getWidth() * 1.5f < group.getGridImage().getX() + group.getGridImage().getWidth()) {
            SmallWaterActor smallWaterActor = new SmallWaterActor(group.getGridImage(), SmallWaterActor.SmallWaterMoveDirection.RIGHT);
            setSmallWaterActorProperty(smallWaterActor);
            System.out.println("R");
        }
        
        if (getY() - getHeight() / 2.0f > group.getGridImage().getY()) {
            SmallWaterActor smallWaterActor = new SmallWaterActor(group.getGridImage(), SmallWaterActor.SmallWaterMoveDirection.DOWN);
            setSmallWaterActorProperty(smallWaterActor);
        }
        
        if (getY() + getHeight() * 1.5f < group.getGridImage().getY() + group.getGridImage().getHeight()) {
            SmallWaterActor smallWaterActor = new SmallWaterActor(group.getGridImage(), SmallWaterActor.SmallWaterMoveDirection.UP);
            setSmallWaterActorProperty(smallWaterActor);
        }
    }
    ...
    public void setSmallWaterActorProperty(SmallWaterActor smallWaterActor) {
        ...
        if (smallWaterActor.moveDirection == SmallWaterActor.SmallWaterMoveDirection.UP ||
                smallWaterActor.moveDirection == SmallWaterActor.SmallWaterMoveDirection.DOWN) {
